...
const OsamaniaBlock = ({ inventory, setInventory, isAuthenticated, contentRef }) => {
  ...
  useEffect(() => {
    const loadRecipes = async () => {
      setLoading(true);
      try {
        const recipes = await loadCustomRecipes('osamania');
        ...
        console.log(`✅ Loaded ${recipes.length} recipes for OsamaniaBlock`);
  ...
  const refreshRecipes = async () => {
    try {
      ...
      console.log('🔄 Calling loadCustomRecipes with category: osamania');
  ...
  const handlePrintBlock = async () => {
    ...
    try {
      ...
      await printRecipeBlock('OSAMANIA', blockDataWithQuantities, () => {
        setIsPrinting(false);
        console.log('✅ Osamania block printed successfully');
      });
    } catch (error) {
  ...
  const handleAddRecipe = async (newRecipe) => {
    ...
    await saveCustomRecipes('osamania', [newRecipe, ...customRecipes], true); // Skip MongoDB save to prevent duplicates
    ...
    try {
      ...
      const recipeData = {
        ...
        description: `Recipe from Osamania Block: ${newRecipe.description}`,
        category: "osamania",
  ...
  const handleAddRow = async (newRow, subCategory) => {
    ...
    // Save to localStorage for immediate access (but don't display it yet)
    await saveCustomRecipes('osamania', [recipeWithSubCategory, ...customRecipes], true); // Skip MongoDB save to prevent duplicates
    ...
    try {
      ...
      const recipeData = {
        ...
        description: `Recipe row from Osamania Block: ${newRow.description || newRow.name}`,
        category: "osamania",
        subCategory: subCategory,
        tableStructure: {
          columns: [
  ...
  const handleEditRecipe = async (updatedRecipe) => {
    ...
    await saveCustomRecipes('osamania', updatedRecipes, true); // Skip MongoDB save to prevent duplicates
    ...
    if (updatedRecipe.mongoId) {
      try {
        ...
        const recipeData = {
          ...
          description: `Recipe from Osamania Block: ${updatedRecipe.description}`,
          category: "osamania",
