...
const QameerBlock = ({ inventory, setInventory, isAuthenticated, contentRef }) => {
  ...
  useEffect(() => {
    const loadRecipes = async () => {
      setLoading(true);
      try {
        const recipes = await loadCustomRecipes('qameer');
        ...
        console.log(`✅ Loaded ${recipes.length} recipes for QameerBlock`);
  ...
  const refreshRecipes = async () => {
    try {
      ...
      console.log('🔄 Calling loadCustomRecipes with category: qameer');
  ...
  const handlePrintBlock = async () => {
    ...
    try {
      ...
      await printRecipeBlock('QAMEER', blockDataWithQuantities, () => {
        setIsPrinting(false);
        console.log('✅ Qameer block printed successfully');
      });
    } catch (error) {
  ...
  const orderedRecipes = useMemo(() => {
    console.log('🔄 orderedRecipes recalculating, customRecipes count:', customRecipes.length);
    console.log('🔄 customRecipes:', customRecipes);
    const sorted = getSortedRecipes(customRecipes);
  ...
  const recipesBySubCategory = useMemo(() => {
    const grouped = {};
    console.log('🔄 Grouping recipes by subcategory. Total recipes:', orderedRecipes.length);
    console.log('🔄 All recipes:', orderedRecipes);
    
    orderedRecipes.forEach(recipe => {
      const subCategory = recipe.subCategory || 'Uncategorized';
      console.log(`🔄 Processing recipe: ${recipe.description} -> subCategory: ${subCategory}`);
    ...
    console.log('🔄 Final grouped recipes:', grouped);
    return grouped;
  }, [orderedRecipes]);
  ...
  const handleAddRecipe = async (newRecipe) => {
    ...
    await saveCustomRecipes('qameer', [newRecipe, ...customRecipes], true); // Skip MongoDB save to prevent duplicates
    ...
    try {
      ...
      if (newRecipe.ingredients && Array.isArray(newRecipe.ingredients)) {
        newRecipe.ingredients.forEach(ing => {
          if (ing.name && (ing.baseQty !== undefined || ing.qty !== undefined)) {
            // Use baseQty if available, otherwise fall back to qty
            const qty = parseFloat(ing.baseQty || ing.qty) || 0;
            ingredientValues[ing.name] = qty;
            console.log(`✅ Processing ingredient: ${ing.name} = ${qty} (base quantity)`);
          } else {
      ...
      const recipeData = {
        ...
        description: `Recipe from Qameer Block: ${newRecipe.description}`,
        category: "qameer",
  ...
  const handleAddRow = async (newRow, subCategory) => {
    ...
    // Save to localStorage for immediate access (but don't display it yet)
    await saveCustomRecipes('qameer', [recipeWithSubCategory, ...customRecipes], true); // Skip MongoDB save to prevent duplicates
    ...
    try {
      ...
      const recipeData = {
        ...
        description: `Recipe row from Qameer Block: ${newRow.description || newRow.name}`,
        category: "qameer",
        subCategory: subCategory,
        tableStructure: {
          columns: [
          ...
          ingredientValues: newRow.ingredients ? 
            (Array.isArray(newRow.ingredients) ? 
              newRow.ingredients.reduce((acc, ing) => {
                // Save base quantity, not calculated quantity
                acc[ing.name] = parseFloat(ing.baseQty || ing.qty || ing.quantity) || 0;
                return acc;
              }, {}) :
              newRow.ingredients
            ) : {}
      ...
      if (response.ok) {
        ...
        // Refresh recipes from MongoDB to get the updated data structure
        console.log('🔄 Calling refreshRecipes...');
        await refreshRecipes();
  ...
  const handleEditRecipe = async (updatedRecipe) => {
    ...
    await saveCustomRecipes('qameer', updatedRecipes, true); // Skip MongoDB save to prevent duplicates
    ...
    if (updatedRecipe.mongoId) {
      try {
        ...
        const recipeData = {
          ...
          description: `Recipe from Qameer Block: ${updatedRecipe.description}`,
          category: "qameer",
  ...
  const handleDeleteRecipe = async (recipeId) => {
    ...
    // Save to localStorage for immediate access
    await saveCustomRecipes('Qameer Block', updatedRecipes, true); // Skip MongoDB save to prevent duplicates
