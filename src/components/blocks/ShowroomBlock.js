...
const ShowroomBlock = ({ inventory, setInventory, isAuthenticated, contentRef }) => {
  ...
  useEffect(() => {
    const loadRecipes = async () => {
      setLoading(true);
      try {
        const recipes = await loadCustomRecipes('showroom');
        ...
        console.log(`✅ Loaded ${recipes.length} recipes for Showroom Block`);
  ...
  const refreshRecipes = async () => {
    try {
      ...
      console.log('🔄 Calling loadCustomRecipes with category: showroom');
  ...
  const handlePrintBlock = async () => {
    ...
    try {
      ...
      await printRecipeBlock('SHOWROOM', blockDataWithQuantities, () => {
        setIsPrinting(false);
        console.log('✅ Showroom block printed successfully');
      });
    } catch (error) {
  ...
  const handleAddRecipe = async (newRecipe) => {
    ...
    await saveCustomRecipes('showroom', [newRecipe, ...customRecipes], true); // Skip MongoDB save to prevent duplicates
    ...
    try {
      ...
      const recipeData = {
        ...
        description: `Recipe from Showroom Block: ${newRecipe.description}`,
        category: "showroom",
  ...
  const handleAddRow = async (newRow, subCategory) => {
    ...
    // Save to localStorage for immediate access (but don't display it yet)
    await saveCustomRecipes('showroom', [recipeWithSubCategory, ...customRecipes], true); // Skip MongoDB save to prevent duplicates
    ...
    try {
      ...
      const recipeData = {
        ...
        description: `Recipe row from Showroom Block: ${newRow.description || newRow.name}`,
        category: "showroom",
        subCategory: subCategory,
        tableStructure: {
          columns: [
  ...
  const handleEditRecipe = async (updatedRecipe) => {
    ...
    await saveCustomRecipes('showroom', updatedRecipes, true); // Skip MongoDB save to prevent duplicates
    ...
    if (updatedRecipe.mongoId) {
      try {
        ...
        const recipeData = {
          ...
          description: `Recipe from Showroom Block: ${updatedRecipe.description}`,
          category: "showroom",
